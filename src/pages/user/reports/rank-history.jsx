    ...
    <Box>
      {loading ? <Skeleton height={200} /> : (
        <>
          ...
          <Card sx={{ ...cs, p: 2.5, mb: 2 }}>
            <Stack direction={{ xs: "column", md: "row" }} spacing={3}>
              <Box sx={{ flex: 1, textAlign: "center", py: 2 }}>
                ...
                <Typography sx={{ fontSize: "0.75rem", color: MUTED }}>Current Rank</Typography>
                <Typography sx={{ fontSize: "1.5rem", fontWeight: 800, color: ORO }}>{data?.current_rank}</Typography>
              </Box>
              {nr && (
                <Box sx={{ flex: 2, py: 2 }}>
                  <Typography sx={{ fontSize: "0.85rem", fontWeight: 700, color: ESPRESSO, mb: 1 }}>Next Rank: {nr.name}</Typography>
                  <Box mb={1.5}>
                    ...
                    {nr.pqv_remaining > 0 && (
                      <Typography sx={{ fontSize: "0.72rem", color: MUTED, mt: 0.5 }}>Missing: <b style={{ color: ORO }}>{nr.pqv_remaining} QV</b></Typography>
                    )}
                  </Box>
          ...
          {/* History */}
          <Card sx={{ ...cs, p: 2.5 }}>
            <Typography sx={{ fontSize: "0.9rem", fontWeight: 700, color: ESPRESSO, mb: 2 }}>Rank History</Typography>
                ...
                <TableBody>
                  {(data?.history || []).map((h) => (
                    <TableRow key={h.id}>
                      ...
                      <TableCell>
                        <Chip label={h.is_downgraded ? "Downgrade" : "Upgrade"} size="small"
