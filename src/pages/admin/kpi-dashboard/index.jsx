...
const PeriodFilter = ({ period, setPeriod, customFrom, setCustomFrom, customTo, setCustomTo }) => {
  ...
  return (
    <Stack direction="row" alignItems="center" spacing={1.5} flexWrap="wrap">
      <ButtonGroup size="small" sx={{ "& .MuiButton-root": { textTransform: "none", fontSize: "0.82rem", fontWeight: 600, borderColor: alpha(ORO, 0.3) } }}>
        {periods.map((p) => (
      ...
      {period === "custom" && (
        <Stack direction="row" spacing={1}>
          <TextField size="small" type="date" value={customFrom} onChange={(e) => setCustomFrom(e.target.value)} sx={{ "& input": { fontSize: "0.82rem", py: 0.5 } }} />
          <TextField size="small" type="date" value={customTo} onChange={(e) => setCustomTo(e.target.value)} sx={{ "& input": { fontSize: "0.82rem", py: 0.5 } }} />
        </Stack>
      )}
    </Stack>
  );
...
// SHARED
// ═══════════════════════════════════════
const Section = ({ children }) => <Typography sx={{ fontSize: "0.82rem", fontWeight: 700, color: MUTED, textTransform: "uppercase", letterSpacing: 1.2, mb: 1.5, mt: 2 }}>{children}</Typography>;
const Insight = ({ color = WARNING, icon = "mdi:lightbulb-outline", children }) => (
  <Box sx={{ mt: 1.5, p: 1.5, bgcolor: alpha(color, 0.06), borderRadius: 2, border: `1px solid ${alpha(color, 0.15)}`, display: "flex", alignItems: "flex-start", gap: 1 }}>
    ...
    <Typography sx={{ fontSize: "0.75rem", color: TEXT, lineHeight: 1.4 }}>{children}</Typography>
  </Box>
);
  ...
  return (
    <Box sx={{ mb: 1.5 }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={0.3}>
        <Typography sx={{ fontSize: "0.8rem", color: TEXT, fontWeight: 600 }}>{label}</Typography>
        <Stack direction="row" alignItems="center" spacing={0.5}>
          <Typography sx={{ fontSize: "0.8rem", color, fontWeight: 700 }}>{value}{suffix}</Typography>
          {badge && <Chip label={badge} size="small" sx={{ height: 16, fontSize: "0.62rem", fontWeight: 700, bgcolor: alpha(color, 0.1), color }} />}
        </Stack>
      </Stack>
          ...
          if (isDaily) {
            ...
            return (
              <Box key={i} sx={{ flex: 1, textAlign: "center", display: "flex", flexDirection: "column", justifyContent: "flex-end", height: "100%" }}>
                ...
                <Typography sx={{ fontSize: "0.62rem", color: MUTED, mt: 0.3 }}>{d.label}</Typography>
              </Box>
            );
          }
        })}
      ...
      <Stack spacing={1}>
        {segments.map((s) => (
          <Stack key={s.label} direction="row" alignItems="center" spacing={1}>
            ...
            <Typography sx={{ fontSize: "0.8rem", color: TEXT, fontWeight: 600, flex: 1 }}>{s.label}</Typography>
            <Typography sx={{ fontSize: "0.75rem", fontWeight: 700, color: s.color }}>€{s.value.toLocaleString("it")} ({s.pct}%)</Typography>
          </Stack>
        ))}
      </Stack>
        ...
        <Box sx={{ mt: 1.5, p: 1.5, bgcolor: alpha(ORO, 0.04), borderRadius: 2 }}>
          ...
          {rb.kits_detail.map((k, i) => (
            <Stack key={i} direction="row" justifyContent="space-between" sx={{ py: 0.2 }}>
              ...
              <Typography sx={{ fontSize: "0.7rem", fontWeight: 700, color: TEXT }}>{k.cnt}x · €{Number(k.rev).toLocaleString("it")}</Typography>
            </Stack>
          ))}
        </Box>
      )}
        ...
        {[
          ...
          <Grid item xs={6} key={m.label}>
            <Box sx={{ p: 1.5, bgcolor: alpha(m.color, 0.04), borderRadius: 2, border: `1px solid ${alpha(m.color, 0.1)}`, textAlign: "center" }}>
              ...
              <Typography sx={{ fontSize: "0.65rem", color: MUTED }}>{m.label}</Typography>
            </Box>
          </Grid>
        ))}
      </Grid>
            ...
            <Box key={c.type}>
              <Stack direction="row" justifyContent="space-between" alignItems="center" mb={0.3}>
                <Stack direction="row" alignItems="center" spacing={0.5}>
                  ...
                  <Typography sx={{ fontSize: "0.78rem", color: TEXT, fontWeight: 600 }}>{c.label}</Typography>
                </Stack>
                <Typography sx={{ fontSize: "0.75rem", fontWeight: 700, color: clr }}>€{c.total} ({c.pct}%)</Typography>
              </Stack>
      ...
      <Stack spacing={0.8}>
        {segments.map((s) => (
          <Stack key={s.label} direction="row" alignItems="center" spacing={1}>
            ...
            <Typography sx={{ fontSize: "0.78rem", color: TEXT, fontWeight: 600, flex: 1 }} noWrap>{s.label}</Typography>
            <Typography sx={{ fontSize: "0.7rem", color: MUTED }}>{s.orders}x</Typography>
            <Typography sx={{ fontSize: "0.75rem", fontWeight: 700, color: TEXT }}>{s.pct}%</Typography>
          </Stack>
        ))}
      </Stack>
    </Card>
      ...
      <Stack spacing={1.5}>
        {geo.slice(0, 5).map((g) => (
          <Stack key={g.country} direction="row" alignItems="center" spacing={1}>
            <Typography sx={{ fontSize: "0.8rem", fontWeight: 700, color: TEXT, width: 24 }}>{(g.country || "N/A").substring(0, 2).toUpperCase()}</Typography>
            <Typography sx={{ fontSize: "0.78rem", color: TEXT, flex: 1 }}>{g.country || "N/A"}</Typography>
            ...
            <Typography sx={{ fontSize: "0.75rem", fontWeight: 700, color: TEXT }}>{total > 0 ? Math.round(g.cnt / total * 100) : 0}%</Typography>
          </Stack>
        ))}
      </Stack>
    </Card>
    ...
    <Card sx={{ ...cardSx, p: 3, height: "100%" }}>
      ...
      <Stack spacing={1}>
        {segments.map((s) => (
          <Stack key={s.label} direction="row" alignItems="center" spacing={1}>
            <Box sx={{ width: 8, height: 8, borderRadius: "50%", bgcolor: s.color }} />
            <Typography sx={{ fontSize: "0.78rem", color: TEXT, flex: 1 }}>{s.label}</Typography>
            <Typography sx={{ fontSize: "0.78rem", fontWeight: 700, color: TEXT }}>{s.value}</Typography>
            <Chip label={`${total > 0 ? Math.round(s.value / total * 100) : 0}%`} size="small" sx={{ height: 18, fontSize: "0.65rem", fontWeight: 700, bgcolor: alpha(s.color, 0.1), color: s.color }} />
          </Stack>
        ))}
      </Stack>
      ...
      <Stack spacing={0.8}>
        {top5.map((t, i) => (
          <Stack key={t.user_id} direction="row" alignItems="center" spacing={1}>
            ...
            <Typography sx={{ fontSize: "0.8rem", fontWeight: 600, color: TEXT, flex: 1 }} noWrap>{t.name || t.username}</Typography>
            ...
            <Typography sx={{ fontSize: "0.75rem", fontWeight: 700, color: ORO, width: 35, textAlign: "right" }}>{t.pct}%</Typography>
          </Stack>
        ))}
      ...
      <Stack spacing={0.5}>
        ...
        {grid.map((row, si) => (
          <Stack key={si} direction="row" spacing={0.5} alignItems="center">
            <Box sx={{ width: 45 }}><Typography sx={{ fontSize: "0.58rem", color: MUTED }}>{HOUR_SLOTS[si]}</Typography></Box>
            {row.map((cnt, di) => { const int = cnt / mx; return (
              <Box key={di} sx={{ flex: 1, height: 32, borderRadius: 1.5, bgcolor: cnt > 0 ? alpha(ORO, 0.08 + int * 0.55) : "#fafafa", display: "flex", alignItems: "center", justifyContent: "center", border: si === pS && di === pD ? `2px solid ${ORO}` : "none" }}>
                <Typography sx={{ fontSize: "0.7rem", fontWeight: cnt > 0 ? 700 : 400, color: int > 0.6 ? "#fff" : cnt > 0 ? TEXT : "#ddd" }}>{cnt || ""}</Typography>
              </Box>);
            })}
          </Stack>
        ))}
        ...
        {items.map((t, i) => (
          <Stack key={t.user_id || i} direction="row" alignItems="center" spacing={1}>
            ...
            <Box sx={{ flex: 1, minWidth: 0 }}>
              <Typography sx={{ fontSize: "0.8rem", fontWeight: 600, color: TEXT }} noWrap>{t.name || t.username}</Typography>
              {t.rank_name && <Typography sx={{ fontSize: "0.62rem", color: ORO }}>{t.rank_name}</Typography>}
            </Box>
            <Box sx={{ textAlign: "right" }}>
              <Typography sx={{ fontSize: "0.82rem", fontWeight: 700, color: ORO }}>{typeof t[valueKey] === "number" ? `${valueSuffix}${t[valueKey]}` : t[valueKey]}</Typography>
              {secondaryKey && <Typography sx={{ fontSize: "0.62rem", color: MUTED }}>{t[secondaryKey]} {secondaryLabel}</Typography>}
            </Box>
          </Stack>
        ))}
      </Stack>
