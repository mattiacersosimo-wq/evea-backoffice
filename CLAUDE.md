...
- **Bonifico: ordini sospesi fino all'incasso (27/08/2026, commit backend `ba2327f8`)**: attivato il pagamento con bonifico su Shopify, che consegna l'ordine con `financial_status: pending`. Il backend non guardava quel campo — `mapShopifyCancelStatus` gestisce solo `cancelled`/`refunded`/`voided` — quindi un ordine non pagato veniva trattato come completo. Caso reale: ordine 8340012728666 (Founder Pack €1100 di Angelob.90) ha generato la commissione FSB €50 allo sponsor 1399 e la mail di preparazione a DCA senza che i soldi fossero arrivati; con l'auto-approve a 15 giorni quei €50 sarebbero diventati esigibili l'11/09. **Regola**: l'ordine si registra normalmente (cart, `user_payments`, pacchetto, accesso backoffice) ma restano sospesi commissioni, notifica magazzino e coupon ROB. Le commissioni vanno in `payment_status = 'awaiting_payment'` — stato nuovo, NON `on_hold` che significa già "sponsor non qualificato": `commission:auto-approve` pesca solo `pending` e i report filtrano `whereIn(['yes','pending','on_hold'])`, quindi restano invisibili al promoter. `payment:process` continua a girare perché assegna anche il pacchetto, non solo le commissioni: si parcheggia il denaro dopo, non si salta il comando. **Eccezione Kit Promotore €79**: non è merce ma la chiave d'accesso, quindi `is_promoter` e backoffice restano chiusi fino all'incasso — su Shopify i metodi di pagamento manuali valgono per l'intero checkout e non si possono escludere per singolo prodotto (verificato), quindi il gate deve stare nel backend. **Sblocco**: quando segni l'ordine pagato su Shopify parte un `orders/updated` con `paid` che finiva scartato dalla guardia di idempotenza in `processShopifyCheckout`; ora c'è un ramo valutato prima dello skip (`releasePendingPayment`) che riattiva tutto. **Scadenza 5 giorni lavorativi** (non 3): il contatore misura la catena umana — bonifico disposto, accreditato, segnato pagato — non il tempo tecnico del SEPA che è D+1; con 3 giorni un ordine del giovedì sera scade il martedì e si rischia di annullare ordini già pagati ma non ancora riconciliati. Cron `orders:check-pending-payments` alle 09:15: sollecito al cliente a 2 giorni dalla scadenza, congelamento in `payment_expired` a termine scaduto (reversibile), riepilogo a `PENDING_PAYMENT_ALERT_EMAIL`. Nessun flag utente revocato in automatico: il declassamento di chi non paga il kit resta manuale e compare nel riepilogo.
- **`recurring_orders` non è mai esistita (27/08/2026, commit backend `88582214`)**: `push:backoffice-smartship-events` moriva alla prima query e non ha mai inviato nulla; `push:backoffice-customer-events` moriva sulla terza sezione dopo che le prime due avevano già spedito. Due `production.ERROR` al giorno che sporcavano il log. La tabella doveva essere il mirror locale delle sottoscrizioni **Seal** ma non è mai stata costruita — non esistono né la tabella né una migration che la crei. I dati SmartShip stanno in Seal (`seal_api_logs`) e nei flag su `users` (`is_smartship_customer`, `smartship_activated_at`, `has_smartship_discount`). Messa una guardia `Schema::hasTable` con `Log::warning` esplicito. Oggi `is_smartship_customer=1` su zero utenti, quindi non si perde nessuna notifica; quando la SmartShip partirà davvero va costruito il mirror o riscritte le query sull'API Seal.
